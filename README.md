...
- **打开网页这件事交给 dsh 自己做**：启动器默认不再传 `--no-open`，由 dsh 用它自己的
  "默认浏览器交接"打开带 token 的地址。也就是说，**即使 dsh 以后改了启动横幅的措辞，
  网页照样能打开** —— 启动器不再依赖解析那行文字来完成关键动作。
- 启动器仍然会**顺手**抓一下 `dsh web: <带 token 的地址>` 这行，只用于两件事：
  ① 记进 `%USERPROFILE%\.dsh\launcher-session.json`；② 万一 dsh 报告交接失败，由启动器兜底打开。
- 因为 token 不落盘，**复用已有实例**（不重启服务、只开浏览器）时必须靠这份记录；
  没有记录时会打开无 token 地址并提示你去实例窗口里复制链接（浏览器有 cookie 时通常也能直接用）。

> 如果你是在**旧版启动器**拉起的实例上遇到"网页打不开"：关掉那个 DSH 窗口，
> 用桌面快捷方式重新打开一次即可（新实例会重新记录 token）。
> 也可以手动到 DSH 窗口里复制 `dsh web:` 开头的整行链接打开一次。

### 以后升级 dsh 会不会又出问题？

**同一个坑不会**（token 这件事已经用"让 dsh 自己开浏览器"绕开了）。但启动器和 dsh 之间仍有
两处"契约"，它们变了就会出问题，所以启动器现在会**主动预警**：

| 契约 | 变了会怎样 | 现在怎么提前发现 |
|---|---|---|
| `dsh web` 的参数（`--host` / `--port` / `--no-open`）被改名或删除 | 启动直接失败 | `--selftest` 会跑 `dsh web --help` 核对参数，缺了就报"参数契约异常" |
| 启动横幅的措辞 | 只影响"复用实例"和兜底，**不影响首次打开网页** | 运行时会提示没抓到地址，照提示手动打开即可 |
| `dsh web` 子命令 / profile 结构变了 | 启动失败 | 进程提前退出时会打印输出并提示"dsh 的 CLI 参数可能变了" |

所以以后升级 dsh 后如果启动异常，**先跑一次 `DeepSeekHarness.exe --selftest`**，
它会打印 dsh 版本、启动命令、参数契约检查结果，一眼就能看出是启动器跟不上还是 dsh 自身的问题。
